...
VOID STACALL ReceiveData(DDS_DataReader* pstDataReader)
{
	...
	if (DDS_RETCODE_NO_DATA != retCode)
	{
		for (dataNum = 0; dataNum < stDataSeq.length; dataNum++)
		{
			...
			g_numpkg++;
			printf("********** g_numpkg = %d, len = %d  %x \n", g_numpkg, strlen(userData), userData[0]);

			if (dataLen > 10000)
			{
				for (int i = dataLen; i >0; i = i - 10000)
				{
					printf("%c ", (unsigned char)userData[i/10000]);
				}
				printf("\n");
			}
			//for (int i = 0; i < dataLen; i++)
			//{
			//	printf("%x ", (unsigned char)userData[i]);
			//}
			//printf("\n");
		}
		/* 释放缓存 */
...
int main()
{
	...
	//stReaderQos.deadline.period.sec = 0x7fffffff;
	//stReaderQos.deadline.period.sec = 0xffffffff;
	//stReaderQos.history.depth = 20;
	/* 设置历史缓存

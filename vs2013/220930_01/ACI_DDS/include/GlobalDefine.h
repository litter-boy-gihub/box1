...
//#define USE_STATIC_MALLOC_T   //用户静态内存，固定报文长度 
...
// 单线程模式   FC接收由于没有阻塞  需要使用单线程模式
#if defined(RAPID_IO_CHANNEL) || defined _VXWORKS_653 || defined _FC
...
/*********************** 四、定制功能 ********************************/
#define QYDDS_RTI_STRINGHEADER
#define _TOPIC_INFO

//#define STATIC_DISCOVERY      //静态订阅发布
//#define TG					//历史消息合包、非紧急消息合包
//#define NETGATE_TRANSFORM       //转发
/* #define DDS_MONITOR */
...
#if defined _TM1 || defined _TM3
/*
typedef ACoreOs_id		DDS_MUTEX;												/* 定义锁 *--/
#define DDS_MUTEX_INIT(v)		_DO_WHILE_0({ ACoreOs_semaphore_create("semname",1,ACOREOS_MUTEX_SEMAPHORE,0,&v); }) 	/* 初始化锁 *--/
#define DDS_MUTEX_LOCK(v)		ACoreOs_semaphore_obtain(v,ACOREOS_WAIT,ACOREOS_FOREVER);							/* 加锁 *--/
#define DDS_MUTEX_UNLOCK(v)		ACoreOs_semaphore_release(v);							/* 解锁 *--/
#define DDS_MUTEX_DESTROY(v)	//semDelete(v);*/
typedef SEMAPHORE_ID_TYPE		DDS_MUTEX;												/* 定义锁 */
#define DDS_MUTEX_INIT(v)		_DO_WHILE_0({ RETURN_CODE_TYPE returnCode; CREATE_SEMAPHORE("semname", 1, 1, 0, &v, &returnCode); }) 	/* 初始化锁 */
#define DDS_MUTEX_LOCK(v)		_DO_WHILE_0({ RETURN_CODE_TYPE returnCode; WAIT_SEMAPHORE(v, -1, &returnCode);	 })						/* 加锁 */
#define DDS_MUTEX_UNLOCK(v)		_DO_WHILE_0({ RETURN_CODE_TYPE returnCode; SIGNAL_SEMAPHORE(v, &returnCode); })							/* 解锁 */
#define DDS_MUTEX_DESTROY(v)	//semDelete(v);
...
#define PROCESSNUM 2
#define t_4sec      ((SYSTEM_TIME_TYPE)     4000000000ll)

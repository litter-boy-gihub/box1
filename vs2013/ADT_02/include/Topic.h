...
typedef struct _Topic
{
	...
	/****************** 主题关联的发布、订阅端列表 *****************/
	DDS_DataWriter*        pstLocalWriter;	 /* 本地写入器链表 */
	DDS_DataReader*        pstLocalReader;   /* 本地读取器链表 */
	DDS_DiscoveredWriter*  pstDiscWriter;    /* 远端写入器链表 */
	DDS_DiscoveredReader*  pstDiscReader;    /* 远端读取器链表 */
	
	/************************** 行为信息 **************************/
	HistoryCache           stUserData;		 /* 提交后的用户数据 */
	Locator_t*             pstMonLocator;    /* 监控工具节点地址 */
    
    struct _DomainParticipant* pstParticipant; /* 父节点Participant */   
    struct _Topic*		   pNext;            /* 下个节点指针 */
}DDS_Topic;
